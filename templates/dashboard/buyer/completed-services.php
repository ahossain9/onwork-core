<div class="white-padding mb-4">
  <h2 class="mb-0"><?php echo esc_html__('Completed Services', 'prolancer'); ?></h2>
</div>
...
<div class="white-padding">
  ...
  if (is_user_logged_in()) {
    ...
    $total = $wpdb->get_var("SELECT COUNT(*) FROM ${table} WHERE `buyer_id` = ${buyer_id} AND `status` ='complete'");
    $items_per_page = get_option('posts_per_page');
    $page = isset($_GET['cpage']) ? abs((int) $_GET['cpage']) : 1;
    $offset = ($page * $items_per_page) - $items_per_page;

    if ($wpdb->get_var("SHOW TABLES LIKE '${table}'") == $table) {
      $query = "SELECT * FROM ${table} WHERE `buyer_id` = ${buyer_id} AND `status` ='complete' ORDER BY timestamp DESC LIMIT ${offset}, ${items_per_page}";
            ...
            <tr>
              ...
              <td><a href="<?php if (function_exists('onwork_get_page_url_by_template')) {
                            ...
                            if (get_option('permalink_structure')) {
                              ...
                              echo "&";
                            } ?>fed=completed-service-details&order_id=<?php echo esc_attr($result['id']); ?>" class="prolancer-btn small-btn text-white"><?php echo esc_html__('Details', 'prolancer'); ?></a></td>
            </tr>
          <?php } ?>
        </tbody>
      </table>
