		...
		<div class="row">
			...
			<div class="col-md-6 mb-4">
				<select name="project_type" class="form-control">
					...
					<option <?php selected( get_post_meta( $project_id, 'project_type', true ), 'Fixed' ) ?> value="<?php echo esc_attr( 'Fixed' ); ?>"><?php echo esc_html( 'Fixed','prolancer' ); ?></option>
					<option <?php selected( get_post_meta( $project_id, 'project_type', true ), 'Hourly' ) ?> value="<?php echo esc_attr( 'Hourly' ); ?>"><?php echo esc_html( 'Hourly','prolancer' ); ?></option>
				</select>
			</div>			
			<div class="col-md-6 mb-4">
			...
			<div class="col-md-12 mb-4">
				...
				<select name="skills[]" class="form-control multiple-select" multiple="multiple">
					...
					foreach ($terms as $term) { ?>
						<option value="<?php echo esc_attr($term->term_id) ?>" <?php if(isset($current)){if(in_array($term->term_id , $current) ){echo'selected ="selected"';}} ?>><?php echo esc_html($term->name) ?></option>
					<?php }
					?>
				</select>
			</div>
			...
			<div class="col-md-12 mb-4">
				...
				<input name="featured_project" id="featured" type="checkbox" class="form-check-input" <?php if(get_post_meta($project_id, 'featured_project', true) == true ){echo'checked';} ?>>
				<i class="d-block">
					...
					<?php if (function_exists('prolancer_get_currency_symbol')) {
			           echo esc_html(prolancer_get_currency_symbol($prolancer_project_featuring_fee));
			        } ?>
			    </i>
			</div>
			<div class="col-md-12">
