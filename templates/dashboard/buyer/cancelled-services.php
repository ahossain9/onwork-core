...
<div class="white-padding">
  ...
  if( is_user_logged_in() ){
    ...
    if($wpdb->get_var("SHOW TABLES LIKE ${table}") == $table) {
      $query = "SELECT * FROM ${table} WHERE `buyer_id` = ${buyer_id} AND `status` ='cancel' ORDER BY timestamp DESC ${offset}, ${items_per_page}";
    ...
    <table class="prolancer-table">
      ...
      ?>
        ...
        <tr>
          ...
          <td><a href="<?php if(function_exists('prolancer_get_page_url_by_template')){ echo esc_url(prolancer_get_page_url_by_template('prolancer-dashboard.php'));} if(get_option('permalink_structure')){echo"?";}else{echo"&";} ?>fed=disputes" class="prolancer-btn bg-danger small-btn text-white"><?php echo esc_html__( 'Open dispute', 'prolancer' ); ?></a></td>
        </tr>
        <?php } ?>
      </tbody>
    </table>
