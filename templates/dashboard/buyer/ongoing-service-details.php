...
if (is_user_logged_in()) {
  ...
  global $wpdb;
  $table = 'onwork_service_orders';

  if ($wpdb->get_var("SHOW TABLES LIKE '${table}'") == $table) {
    $query = "SELECT * FROM ${table} WHERE `id` = ${order_id} AND `buyer_id` = ${buyer_id} AND `status` ='ongoing' ORDER BY timestamp DESC LIMIT 1";
...
<?php if ($result) { ?>
      ...
      <table class="prolancer-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><?php if (get_term_by('id', $delivery_time_id, 'delivery-time')) {
                  echo esc_html(get_term_by('id', $delivery_time_id, 'delivery-time')->name);
                } ?></td>
            <td>
              <?php if (function_exists('onwork_get_currency_symbol')) {
                echo esc_html(onwork_get_currency_symbol($result['service_price']));
              } ?>
            </td>
            <td>
            ...
            <td>
              <ul class="list-inline mb-0">
                <li class="list-inline-item">
                  ...
                  <a href="#" class="prolancer-btn small-btn text-white bg-danger" id="service-order-cancel" data-nonce="<?php echo wp_create_nonce('service_order_cancel_nonce'); ?>" data-seller-id="<?php echo esc_attr($seller_id); ?>" data-order-id="<?php echo esc_attr($order_id); ?>">
                      ...
                      <form id="review-form" enctype="multipart/form-data">
                        <div class="modal-content">
                          ...
                          <div class="modal-body">
                            ...
                            <button type="submit" id="service-order-complete" data-nonce="<?php echo wp_create_nonce('service_order_complete_nonce'); ?>" data-seller-id="<?php echo esc_attr($seller_id); ?>" data-order-id="<?php echo esc_attr($order_id); ?>"><?php echo esc_html__('Submit', 'prolancer'); ?></button>
                          </div>
                        </div>
                      </form>
                    </div>
  ...
  if ($wpdb->get_var("SHOW TABLES LIKE '${table}'") == $table) {
    ...
    <form id="send-service-message-form">
      ...
      <a href="#" class="send-service-message prolancer-btn" data-nonce="<?php echo wp_create_nonce('send_service_message_nonce'); ?>" data-order-id="<?php echo esc_attr($order_id) ?>" data-sender-id="<?php echo esc_attr(get_user_meta(get_current_user_id(), 'buyer_id', true)); ?>" data-receiver-id="<?php echo esc_attr($seller_id); ?>"><?php echo esc_html__('Send message', 'prolancer'); ?></a>
    </form>
  <?php }
} else { ?>
  <div class="white-padding">
