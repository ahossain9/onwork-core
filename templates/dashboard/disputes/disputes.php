...
if ($wpdb->get_var("SHOW TABLES LIKE '${table}'") == $table) {
	...
	foreach ($services as $index => $service) {
		...
		$service_price_array[] = $service['service_price'];
	}
}
...
if ($wpdb->get_var("SHOW TABLES LIKE '${table}'") == $table) {
	...
	foreach ($projects as $index => $project) {
		...
		$proposed_price_array[] = $project['proposed_price'];
	}
}
...
if (isset($service_array) & isset($project_array)) {
	...
	$amount_ids = array_merge($service_price_array, $proposed_price_array);
		...
		<div class="col-md-12">
			<div class="disputes white-padding mb-4">
				<form id="disputes-form" enctype="multipart/form-data">
							...
							<select class="mb-4" name="dispute-id">
								...
								if (isset($post_ids)) {
									if (array_unique($post_ids)) {
										foreach (array_unique($post_ids) as $key => $id) { ?>
											<option value="<?php echo esc_attr($id); ?>" data-price="<?php echo esc_attr($amount_ids[$key]); ?>"><?php echo get_the_title($id) . ' (' . onwork_get_currency_symbol($amount_ids[$key]) . ')'; ?></option>
								<?php }
									}
								} ?>
							</select>
					...
					<?php if (isset($amount_ids)) { ?>
						<input type="hidden" name="amount" value="<?php echo esc_attr($amount_ids[0]); ?>">
					<?php } ?>
											...
											<tr>
												...
												<td>
													<?php if (function_exists('onwork_get_currency_symbol')) {
														echo esc_html(onwork_get_currency_symbol(get_post_meta(get_the_ID(), 'dispute_price', true)));
													} ?>
												</td>
												<td>
