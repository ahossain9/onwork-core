<div class="white-padding mb-4">
  <h2 class="mb-0"><?php echo esc_html__( 'complete Services', 'prolancer' ); ?></h2>
</div>
...
<div class="white-padding">
  ...
  if( is_user_logged_in() ){
    ...
    if($wpdb->get_var("SHOW TABLES LIKE ${table}") == $table) {
      $query = "SELECT * FROM ${table} WHERE `seller_id` = ${seller_id} AND `status` ='complete' ORDER BY timestamp DESC ${offset}, ${items_per_page}";
    ...
    <table class="prolancer-table">
      ...
      <?php
        ...
        <tr>
          ...
          <td><a href="<?php if(function_exists('prolancer_get_page_url_by_template')){ echo esc_url(prolancer_get_page_url_by_template('prolancer-dashboard.php'));} if(get_option('permalink_structure')){echo"?";}else{echo"&";} ?>fed=completed-service-details&order_id=<?php echo esc_attr( $result['id'] ); ?>" class="prolancer-btn small-btn text-white"><?php echo esc_html__( 'Details', 'prolancer' ); ?></a></td>
        </tr>
        <?php } ?>
      </tbody>
    </table>
